...
/**
  ...
  *Version: 1.0 "@(#) functionTracer. header. ver. 1.0. Premalatha and Vidyadhar, CTFD, NAL."
...
class functionTracer
{
   ...
   // manipulators
#if defined(__GNUC__)
      functionTracer& operator <<(std::ostream& (*ptr_manipulator) (std::ostream&))
      {
         if ( pv_ptr_output_channel->pv_ptr_trace_stream )
         {
   ...
   // other data types (looks like we must put the manipulators FIRST!!)
      template <class T> 
      functionTracer& operator <<(T i)
      {
         if ( pv_ptr_output_channel->pv_ptr_trace_stream )
         {
...
/**
  ...
  */
      ...
      ///////////////////////////////////////////////////////////
      //
      //    looks like we do not need all these!
      //
#if 0
   // insert a c string
   ...
   // insert a manipulator
      template <class T>
      functionTracer& operator<< ( std::basic_ostream<char>& (*T) (std::basic_ostream<char>&) ) {
         if ( pv_ptr_output_channel->pv_ptr_trace_stream )
         {
            *pv_ptr_output_channel->pv_ptr_trace_stream << T;
         }
         return *this;
      }
