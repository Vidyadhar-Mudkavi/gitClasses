...
/**
  ...
  *Description:
   ...
   old basis to the new one and vice versa. we adhere to the following
   terminology when referring to "old" and "new" basis. given a basis,
   we define a new basis with respect to this "old" basis. the "old" basis
   is the WORLD coordinate system as far as the "new" basis is concerned.
   we will refer to the "new" basis as the LOCAL basis. this is convenient
   since we can talk of a vector being transformed to "this" basis, i.e.,
   to the local basis. accordingly, you will find the local and world
   as the function names to tranform the vectors back and forth.
   ...
   to "new" and vice-versa use this notion of new and old. (NOTE that we
   have now redefined the old and new to mean world and local respectively).
  ...
  *Version: 1.0 "@(#) aVectorBasis. header. ver. 1.0. Premalatha and Vidyadhar, CTFD, NAL."
...
// output
   ...
   // functions to transform a given vector
      aVector ToLocal(const aVector& v_world) const;  // transform to local system
