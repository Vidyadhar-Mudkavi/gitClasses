...
/**
   exception classes
  */
class a2DVectorException : public anException
{
   public:
      a2DVectorException() {
         Header()  << "Exception raised by a2DVector";
         Trailer() << "*****************************";
      }
   protected:
   private:
...
class a2DVectorOutOfBoundsException : public anException
{
   public:
      a2DVectorOutOfBoundsException() {
         Header()  << "Exception raised by a2DVector" << std::endl
                   << "Out of bounds!";
         Trailer() << "*****************************";
      }
   protected:
...
class a2DVector
{
   typedef a2DVectorException exception;
   typedef a2DVectorOutOfBoundsException outOfBoundsException;
   ...
   // attach object to iostreams for reading and writing
      friend std::ostream& operator<<(std::ostream& os, const a2DVector& v);
      friend std::istream& operator>>(std::istream& is, a2DVector& v);
   ...
   private:
      ...
      bool     pv_bln_true;  // we need these to compare
      bool     pv_bln_false; 
};
