...
// check if checkbounds is working (provided CHECKBOUNDS is on)
   ...
   char dummy_input;
   std::cout << "Pausing. Type something to continue ";
   std::cin >> dummy_input;
...
// pause here
   int dummy;
   std::cout   << std::endl
               << "type some number and return to continue :-)"
               << std::endl;
   std::cin >> dummy;
...
// product is zero.
   ...
   for (i=0; i<100; ++i)
   {
      vec_dummy = p0 + p1 + p2;
      p0 + p1;
      std::cout   << (p0 * p2 * p3 ^ p1 - p3 + p2 + (p1^p2))
                  << std::endl;
   }

#if 0
// write out the number of objects created so far
   aVector::PrintObjectCount();
#endif
