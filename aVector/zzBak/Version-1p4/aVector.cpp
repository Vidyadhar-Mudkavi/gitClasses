...
{
   return aVector( (pv_p[0] + v2.pv_p[0]),
                   (pv_p[1] + v2.pv_p[1]),
                   (pv_p[2] + v2.pv_p[2]) );
}
...
aVector aVector::operator^(const aVector& v2) const
{
   return aVector( (pv_p[1]*v2.pv_p[2] - v2.pv_p[1]*pv_p[2]),
...
#ifdef VECTOR_DEBUG_ON
   out << "VECTOR_DEBUG is on" << endl;
#else
   out << "VECTOR_DEBUG is off" << endl;
#endif

#ifdef VECTOR_CHECKBOUNDS_ON
   out << "VECTOR_CHECKBOUNDS is on" << endl;
#else
   out << "VECTOR_CHECKBOUNDS is off" << endl;
#endif

#ifdef VECTOR_STOP_ON_ERROR
   out << "VECTOR_STOP_ON_ERROR is on" << endl;
#else
   out << "VECTOR_STOP_ON_ERROR is off" << endl;
#endif
}
